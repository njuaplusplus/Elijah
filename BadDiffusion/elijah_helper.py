...
def sample_with_trigger(ckpt, trigger, file_name, R_coef_T, recomp=False, use_ddim=False, save_res_dict=False):
    ...
    if not os.path.isfile(generated_img_ptfile) or recomp:
        if use_ddim:
            ...
            model, noise_sched = DiffuserModelSched.get_pretrained(ckpt=ckpt, clip_sample=clip)
            unet = model.cuda()
            noise_shape = [unet.in_channels, unet.sample_size, unet.sample_size]
        ...
        def gen_samples(init):
            ...
            # imgs = generate_x_t_minus_1_from_x_T(unet, noise_sched, 1, init.cuda())
            # imgs = (imgs/2+0.5).clamp(0, 1)
            # save_image(imgs, './tmp/tmp.png', nrow=4)

            # images_np_tensor = rearrange(torch.from_numpy(images_np).cuda(), 'b h w c -> b c h w')
            # print('(images_np_tensor-imgs).sum()', (images_np_tensor-imgs).sum())
...
def opt_r(ckpt):
    ...
    sample_with_trigger(ckpt, trigger.cpu(), filename, R_coef_T, save_res_dict=True)
