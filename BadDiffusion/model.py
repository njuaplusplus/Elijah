...
from diffusers import UNet2DModel, DDPMScheduler, DDIMScheduler, PNDMScheduler, LMSDiscreteScheduler, ScoreSdeVeScheduler, KarrasVeScheduler, DiffusionPipeline, DDPMPipeline, DDIMPipeline, PNDMPipeline, ScoreSdeVePipeline, LDMPipeline, KarrasVePipeline
...
def batch_sampling_save(sample_n: int, pipeline, path: Union[str, os.PathLike], init: torch.Tensor=None, max_batch_n: int=256, rng: torch.Generator=None, seed=None):
    if init == None:
        if sample_n > max_batch_n:
            replica = sample_n // max_batch_n
            residual = sample_n % max_batch_n
    ...
    for i, batch_sz in enumerate(batch_sizes):
        pipline_res = pipeline(
                    ...
                    return_first_mov=False
                )
        # sample_imgs_ls.append(pipline_res.images)
        save_imgs(imgs=pipline_res.images, file_dir=path, file_name="" if seed is None else f'seed_{seed}_', start_cnt=cnt)
        cnt += batch_sz
        del pipline_res
    # return np.concatenate(sample_imgs_ls)
    return None
...
class DiffuserModelSched():
    ...
    def get_pretrained(ckpt: str, clip_sample: bool=None):        
        if ckpt == DiffuserModelSched.DDPM_CIFAR10_32:
            ...
            print(f'NOTE!!!!!! we are using some new {ckpt} from Hugging Face!')
        # else:
    ...
    @staticmethod
    def __new_get_pipeline_generator(unet, scheduler, pipeline):
        def get_pipeline(unet, scheduler):
            return pipeline(unet, scheduler)
        return get_pipeline

    @staticmethod
    def __new_get_model_sched(ckpt_id: str, clip_sample: bool, noise_sched_type: str=None):
        # Clip option
        clip_sample_used = DiffuserModelSched.get_sample_clip(clip_sample=clip_sample, clip_sample_default=DiffuserModelSched.CLIP_SAMPLE_DEFAULT)
        # Pipeline
        pipline: DDPMPipeline = DDPMPipeline.from_pretrained(ckpt_id)
        ...
        if noise_sched_type == DiffuserModelSched.DDPM_SCHED:
            ...
            get_pipeline = DiffuserModelSched.__new_get_pipeline_generator(unet=model, scheduler=noise_sched, pipeline=DDPMPipeline)
            ...
            get_pipeline = DiffuserModelSched.__new_get_pipeline_generator(unet=model, scheduler=noise_sched, pipeline=DDIMPipeline)
        ...
        #     get_pipeline = DiffuserModelSched.__new_get_pipeline_generator(unet=model, scheduler=noise_sched, pipeline=PNDMPipeline_used)
    ...
    @staticmethod
    def new_get_model_sched(image_size: int, channels: int, model_type: str=MODEL_DEFAULT, noise_sched_type: str=None, clip_sample: bool=None, **kwargs):
        @torch.no_grad()
        def weight_reset(m: nn.Module):
            # - check if the current module has reset_parameters & if it's callabed called it on m
            reset_parameters = getattr(m, "reset_parameters", None)
        ...
        if model_type == DiffuserModelSched.MODEL_DEFAULT:
            ...
            model, noise_sched, get_pipeline = DiffuserModelSched.new_get_pretrained(ckpt=DiffuserModelSched.DDPM_CIFAR10_32, noise_sched_type=noise_sched_type, clip_sample=clip_sample)
            model = model.apply(weight_reset)
        elif model_type == DiffuserModelSched.DDPM_CELEBA_HQ_DEFAULT:
            model, noise_sched, get_pipeline = DiffuserModelSched.new_get_pretrained(ckpt=DiffuserModelSched.DDPM_CELEBA_HQ_256, noise_sched_type=noise_sched_type, clip_sample=clip_sample)
            model = model.apply(weight_reset)
        elif model_type == DiffuserModelSched.DDPM_CHURCH_DEFAULT:
            model, noise_sched, get_pipeline = DiffuserModelSched.new_get_pretrained(ckpt=DiffuserModelSched.DDPM_CHURCH_256, noise_sched_type=noise_sched_type, clip_sample=clip_sample)
            model = model.apply(weight_reset)
        elif model_type == DiffuserModelSched.DDPM_BEDROOM_DEFAULT:
            model, noise_sched, get_pipeline = DiffuserModelSched.new_get_pretrained(ckpt=DiffuserModelSched.DDPM_BEDROOM_256, noise_sched_type=noise_sched_type, clip_sample=clip_sample)
            model = model.apply(weight_reset)
        elif model_type == DiffuserModelSched.LDM_CELEBA_HQ_DEFAULT:
            model, noise_sched, get_pipeline = DiffuserModelSched.new_get_pretrained(ckpt=DiffuserModelSched.LDM_CELEBA_HQ_256, noise_sched_type=noise_sched_type, clip_sample=clip_sample)
            model = model.apply(weight_reset)
        else:
            raise NotImplementedError()
        return model, noise_sched, get_pipeline
    ...
    @staticmethod
    def new_get_pretrained(ckpt: str, clip_sample: bool=None, noise_sched_type: str=None):        
        if ckpt == DiffuserModelSched.DDPM_CIFAR10_32:
            ckpt: str = "google/ddpm-cifar10-32"
        elif ckpt == DiffuserModelSched.DDPM_CELEBA_HQ_256:
            ckpt: str = "google/ddpm-ema-celebahq-256"
        ...
        # return model, noise_sched
        return DiffuserModelSched.__new_get_model_sched(ckpt_id=ckpt, clip_sample=clip_sample, noise_sched_type=noise_sched_type)
    
    @staticmethod
    def new_get_trained(ckpt: str, clip_sample: bool=None, noise_sched_type: str=None):        
        return DiffuserModelSched.__new_get_model_sched(ckpt_id=ckpt, clip_sample=clip_sample, noise_sched_type=noise_sched_type)
