...
def parse_args_and_config():
    ...
    parser.add_argument('--total_n_samples', type=int, default=2048)
    parser.add_argument('--trigger_type', type=str, default='blend')
    parser.add_argument('--patch_size', type=int, default=3)
    parser.add_argument('--subset_ratio', type=float, default=1., help='raito of subset of the training dataset.')
    ...
    parser.add_argument(
        "--bd_but_benign_input",
        action="store_true",
        help="use backdoor sampling with benign input",
    )
    parser.add_argument(
    ...
    if args.merge_backdoor:
        args.exp = args.exp + '_merge_backdoor'

    if args.remove_backdoor:
        args.exp = args.exp + '_remove_backdoor'
    ...
    if not args.test and not args.sample:
        ...
        if args.sample:
            ...
            args.image_folder = os.path.join(
                args.exp, "image_samples", f'{args.image_folder}{"_ema" if new_config.model.ema else ""}'
            )
...
def main():
    ...
    try:
        ...
        if args.remove_backdoor:
            R_coef_T = 0.5
            trigger_filename = f'{runner.args.log_path.replace("_remove_backdoor", "")}/trigger_ckpt{runner.config.sampling.ckpt_id}{"_ema" if runner.config.model.ema else ""}_{R_coef_T}.pt'
