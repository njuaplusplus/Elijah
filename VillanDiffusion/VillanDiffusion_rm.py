...
from dataset_rm import DatasetLoader, Backdoor, ImagePathDataset
...
IGNORE_ARGS = ['overwrite', 'is_save_all_model_epochs', 'R_trigger_only', 'remove_backdoor', ]
...
def setup():
    ...
    wandb.init(project=config.project, name=name_id[:100], id=name_id[:100], settings=wandb.Settings(start_method="fork"))
...
def sampling(config: TrainingConfig, file_name: Union[int, str], pipeline, inverted_trigger=None):
    def gen_samples(init: torch.Tensor, folder: Union[os.PathLike, str], start_from: int=0):
        ...
        if config.ddim_eta == None:
            pipline_res = pipeline(num_inference_steps=config.infer_steps, start_from=start_from, batch_size=config.eval_sample_n, generator=torch.manual_seed(config.seed), init=init, save_every_step=save_gif, output_type=None)
        else:
            pipline_res = pipeline(num_inference_steps=config.infer_steps, start_from=start_from, eta=config.ddim_eta, batch_size=config.eval_sample_n, generator=torch.manual_seed(config.seed), init=init, save_every_step=save_gif, output_type=None)
    ...
    with torch.no_grad():
        ...
        if config.task == TASK_GENERATE:
            ...
            if inverted_trigger is not None:
                ...
                gen_samples(init=init, folder="backdoor_samples_with_inverted", start_from=0)
        else:
            # Special Sampling
...
def update_score_file(config: TrainingConfig, score_file: str, fid_sc: float=None, mse_sc: float=None, ssim_sc: float=None, lpips_sc: float=None, mse_thres_sc=None) -> Dict:
    ...
    if config.task == TASK_GENERATE:
        if fid_sc == None or mse_sc == None or ssim_sc == None or mse_thres_sc is None:
            raise ValueError(f"Task: {config.task} requires FID, MSE, SSIM, and MSE_THRES scores")
        # get_key_fn = get_key_gen
        scores = {"FID": fid_sc, "MSE": mse_sc, "SSIM": ssim_sc, 'MSE_THRES': mse_thres_sc, }
    else:
...
def measure(config: TrainingConfig, accelerator: Accelerator, dataset_loader: DatasetLoader, folder_name: Union[int, str], pipeline, resample: bool=False, recomp: bool=True):
    ...
    if config.dataset == 'CELEBA-HQ-LATENT':
        dataset_img_dir = os.path.join(folder_name, DatasetLoader.CELEBA_HQ)
    ...
    if hasattr(pipeline, 'encode'):
        backdoor_noise = noise + pipeline.encode(dataset_loader.trigger.unsqueeze(0).to(pipeline.device)).to(noise.device)
    else:
        backdoor_noise = noise + dsl.trigger.unsqueeze(0).to(noise.device)
    ...
    if config.task != TASK_GENERATE:
        ...
        if re_comp_backdoor_metric or recomp:
            ...
            if config.sde_type == DiffuserModelSched.SDE_VP or config.sde_type == DiffuserModelSched.SDE_LDM:
                ...
                # NOTE: for sde_ve the target is in [0,1], so no need to denormalize it
                backdoor_target = torch.squeeze((dsl.target.repeat(*reps)).clamp(0, 1)).to(device)
            
            print(f"gen_backdoor_target: {gen_backdoor_target.shape}, vmax: {torch.max(gen_backdoor_target)}, vmin: {torch.min(gen_backdoor_target)} | backdoor_target: {backdoor_target.shape}, vmax: {torch.max(backdoor_target)}, vmin: {torch.min(backdoor_target)}")
            # mse_sc = float(nn.MSELoss(reduction='mean')(gen_backdoor_target, backdoor_target))
            mse_sc = nn.MSELoss(reduction='none')(gen_backdoor_target, backdoor_target).mean(dim=[i for i in range(1, len(gen_backdoor_target.shape))])
            sorted_mse_sc, sorted_mse_ind = torch.sort(mse_sc)
            torch.save(sorted_mse_sc, f'./tmp/{format_ckpt_dir(config.ckpt)}_sorted_mse_sc.pt')
            torchvision.utils.save_image(gen_backdoor_target[sorted_mse_ind], f'./tmp/{format_ckpt_dir(config.ckpt)}_sorted_gen_backdoor.png', nrow=32)
            print('nn.mse_thres_sc', (mse_sc<0.01).float().mean())
            mse_thres_sc = Metric.mse_thres_batch(a=gen_backdoor_target, b=backdoor_target, thres=0.01, max_batch_n=config.eval_max_batch)
            print('mse_thres_sc', mse_thres_sc)
            # print('nn.mse_sc', mse_sc)
...
def train_loop(config: TrainingConfig, accelerator: Accelerator, repo, model: nn.Module, get_pipeline, noise_sched, optimizer: torch.optim, loader, lr_sched, vae=None, start_epoch: int=0, start_step: int=0, inverted_trigger=None):
    ...
    try:
        ...
        sampling(config, 0, pipeline, inverted_trigger)
            ...
            for step, batch in enumerate(loader):
                ...
                with accelerator.accumulate(model):
                    ...
                    if inverted_trigger is not None:  # and deshift_iters < 20:
                        ...
                        # loss1, loss2 = deshift_loss(model, noise, inverted_trigger, noise_sched.num_train_timesteps-1, frozen_model, config)
...
# print(dsl.trigger.unsqueeze(0).cpu().shape)
# torch.save(dsl.trigger.unsqueeze(0).cpu(), 'TRIGGER_STOP_SIGN_14_DDPM.pt')
# torch.save(dsl.trigger.unsqueeze(0).cpu(), 'TRIGGER_GLASSES_LDM.pt')
# torch.save(dsl.trigger.unsqueeze(0).cpu(), 'TRIGGER_STOP_SIGN_LDM.pt')
# exit()
...
if config.mode == MODE_TRAIN or config.mode == MODE_RESUME or config.mode == MODE_TRAIN_MEASURE:
    ...
    pipeline = train_loop(config, accelerator, repo, model, get_pipeline, noise_sched, optimizer, dataloader, lr_sched, vae=vae, start_epoch=cur_epoch, start_step=cur_step, inverted_trigger=inverted_trigger)
