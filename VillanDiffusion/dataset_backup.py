...
class DatasetLoader(object):
    ...
    def prepare_dataset(self, mode: str="FIXED", R_trigger_only: bool=False, ext_R_trigger_only: bool=False, R_gaussian_aug: float=0.0) -> 'DatasetLoader':
        ...
        if self.__name == self.CELEBA_HQ_LATENT:
            self.__full_dataset.set_poison(target_key=self.__target_type, poison_key=self.__trigger_type, raw='raw', poison_rate=self.__poison_rate, use_latent=True, clean_rate=self.__clean_rate).set_use_names(target=DatasetLoader.TARGET, poison=DatasetLoader.PIXEL_VALUES, raw=DatasetLoader.IMAGE)
...
class LatentDataset(torch.utils.data.Dataset):
    ...
    def set_poison(self, target_key: str, poison_key: str, raw: str, poison_rate: float, use_latent: bool=True, clean_rate: float=1.):
        ...
        self.__len = int(self.__clean_rate * self.__len)
        return self
        ...
        def clean_poison(clean_fn: callable, poison_fn: callable):
            def fn(idx: int):
                if idx < int(self.__len * self.__poison_rate):
                    # print(f"Poisoned")
                    # NOTE: here we assert false just to test backdoor_removal
                    # raise AssertionError('We are doing backdoor removal!!')
