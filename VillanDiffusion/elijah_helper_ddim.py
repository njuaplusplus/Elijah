...
def sample_with_trigger(ckpt, trigger, file_name, R_coef_T, recomp=False, use_ddim=False, save_res_dict=False, to_encode_trigger=False):
    ...
    if not os.path.isfile(generated_img_ptfile) or recomp:
        if use_ddim:
            ...
            model, vae, noise_sched, get_pipeline = DiffuserModelSched.get_pretrained(ckpt=ckpt, clip_sample=clip, noise_sched_type=DiffuserModelSched.DDIM_SCHED, sde_type=DiffuserModelSched.SDE_VP)
            unet = model.cuda()
            if vae is not None:
                vae = vae.cuda()
            noise_shape = [unet.in_channels, unet.sample_size, unet.sample_size]
...
def opt_r(ckpt):
    ...
    if not os.path.isfile(trigger_filename):
        ...
        trigger = torch.rand([1, 3, 32, 32]).cuda() * 2 - 1
